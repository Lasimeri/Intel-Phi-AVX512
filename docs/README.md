...
| [2026-09-23-ffn-per-request.md](results/2026-09-23-ffn-per-request.md) | A SwiGLU feed-forward block as one request per card: the intermediate made, used and discarded on the card, `ffn_down` held by columns, partial sums added on the host; SiLU on the vector unit with `vexp223ps` and `vrcp23ps` (5.5e-7 relative error), a masked float16 store, the float16 overflow the conformance found and the guard it led to for all activations; correct on every type combination and token for token on both models, and neutral end to end (27B pp512 14.64 against 14.46 plain), with why |
